...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		self.fwconfig = list()
		self.portSecurityTable = {}
		self.blockedMacTable = []
		self.numberOfMacLimit = 1
	...
	def handleSecurity(self, packet, event=None):
		ovsIngressPort = str(event.port)
		srcMAC = packet.src
		if packet.type == packet.IP_TYPE:
			ipPacket = packet.payload
			srcIP = str(ipPacket.srcip)
			destIP = str(ipPacket.dstip)
			log.debug("New flow with src mac: {} src IP: {} destIP: {} recevied on port {}".format(srcMAC, srcIP, destIP, ovsIngressPort))

			if ovsIngressPort not in self.portSecurityTable:
				log.debug("New table entry with port: {} src mac: {} src IP: {} to port security table".format(ovsIngressPort, srcMAC, srcIP))
				self.portSecurityTable[ovsIngressPort]= {srcMAC: srcIP}
			else:
				MACdata = self.portSecurityTable[ovsIngressPort]
				numOfMAC = len(MACdata)
				if srcMAC not in MACdata and numOfMAC >= self.numberOfMacLimit:
					log.debug("***** MAC spoofing attack deteced, port Security detected number of mac address on port {} reached to the limitation which is {}".format(ovsIngressPort, self.numberOfMacLimit))
	...
	def _handle_PacketIn(self, event):
		...
		if(match.dl_type == packet.IP_TYPE):
			self.handleSecurity(packet, event)
