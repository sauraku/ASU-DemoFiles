...
parser.add_argument("-train", "--Training", help = "Training Dataset")
parser.add_argument("-test", "--Testing", help = "Testing Dataset")

args = parser.parse_args()
 
TrainingData = 'datasets/' + args.Training
TestingData  = 'datasets/' + args.Testing

if(not TrainingData or not TestingData):
    print("Training and Testing Data are required!")
    exit(0)
...
dataset = pd.read_csv(TrainingData, header=None)
...
plt.savefig('images/accuracy_{}+{}.png'.format(TrainingData.replace("datasets/Training-",'').replace('.csv', ''), 
                                               TestingData.replace("datasets/Testing-",'').replace('.csv', '')))
plt.show()
...
plt.xlabel('epoch')
plt.legend(['train'], loc='upper left')
plt.savefig('images/loss_{}+{}.png'.format(TrainingData.replace("datasets/Training-",'').replace('.csv', ''), 
                                           TestingData.replace("datasets/Testing-",'').replace('.csv', '')))
plt.show()
